...
#define LIGHT_SENSOR_PIN_ENV A0  // 环境光传感器引脚
...
#define DFPLAYER_TX D3             // DFPlayer TX引脚 (已连接到D3)
#define DFPLAYER_RX D2             // DFPlayer RX引脚 (已连接到D2)
...
void loop() {
    int lightValueEnv = analogRead(LIGHT_SENSOR_PIN_ENV);  // 环境光传感器值
    int lightValueHat = analogRead(LIGHT_SENSOR_PIN_HAT);  // 帽子光传感器值
    Serial.print("环境光传感器值: "); Serial.println(lightValueEnv);
    //Serial.print("帽子光传感器值: "); Serial.println(lightValueHat);
    delay(100); // 每秒读取一次传感器数据
}